...
export async function promptUserChoose() {
  const { actions, baseURL } = await enquirer.prompt([
    {
      ...
      choices: [
        ...
        {
          message: "Generate QR-codes for each served ZPK file",
          name: "make_qr",
        },
  ...
  if (!baseURL.startsWith("https:")) {
    console.error("");
    console.error(
      "Serve base URL should start with https://, pure HTTP won't work with Zepp application.",
    );
    console.error(
      "Please, provide foll base URL, example: https://example.com/folder",
    );
    process.exit(1);
  }
...
export async function processProject(zabPath, userChoose) {
  ...
  if (actions.includes("patch_deletable")) ZabMakeDeletable.apply(bundle);

  // QR codes
  const mapJson = await SelfHostFolderGenerate.apply(
    bundle,
    baseURL,
    actions.includes("app_id_dir"),
  );
  if (actions.includes("make_qr")) {
    await SelfHostQrGenerate.apply(
      bundle,
      mapJson,
      actions.includes("app_id_dir"),
    );
  ...
  if (bundle.appType === "app") {
    // ОЧИСТКА: Убираем Amazfit из общего списка устройств для корректного сравнения
    const allDevicesRaw = await getAllKnownDeviceNames();
    const allDevices = [
      ...new Set(allDevicesRaw.map((it) => it.replace(/^Amazfit\s+/i, ""))),
    ];
    ...
    for (const dev of allDevices)
      if (!mapped.includes(dev)) unsupported.push(dev);
    ...
    console.log(
      "Ready for devices:",
      Object.keys(mapJson["device_qr"]).join(", "),
    );
  }
}
