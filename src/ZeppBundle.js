...
export class ZeppBundle {
  ...
  get appId() {
    if (!this._cacheAppId) {
      ...
      const randomAppJson = JSON.parse(
        randomFiles["app.json"].toString("utf8"),
      );
  ...
  loadBundle(zabFilePath) {
    ...
    for (const file of bundle.getEntries("")) {
      if (file.entryName.endsWith(".zpk")) this.parsePackage(file);
  ...
  parsePackage(packageFile) {
    ...
    for (const zipItem of pkg.getEntries("")) {
      if (!zipItem.entryName.endsWith(".zip")) {
        console.warn(
          `Unknown member of BUNDLE/${packageFile.entryName}/${zipItem.entryName}, skipping...`,
        );
        continue;
      }
    ...
    if (deviceManifest) {
      this.packagesDeviceManifests[packageFile.entryName] = JSON.parse(
        deviceManifest.toString("utf-8"),
      );
    }
    ...
    if (this.packagesDeviceManifests[packageFile.entryName]) {
      ...
      debugLog(
        `[ZAB EXTRACT] Found manifest for ${packageFile.entryName}. AppID: ${manifest.app?.appId}`,
      );
    }
  }
